...
output_dir = '/data3/zjy/S4NN/output/2_layer/plt_1/'
...
for epoch in range(Nepoch):
    ...
    for raw_frames, label_frame in testdata_loader:
        ...
        for iteration in range(len(test_images)):
            ...
            if epoch in [10,5,16,18,19]:
                ...
                plt.imshow(labels[0, :, :], cmap='viridis', vmax=255, vmin=0)
                plt.colorbar(extend='min')
                plt.suptitle(f'2 Frame Thr=0+L0.5 Epoch {epoch}')
    ...
    # To save the weights
    if saving:
        np.save("/data3/zjy/S4NN/clear_background/log1/weights", W, allow_pickle=True)
        print("Dy-W done.")
        if test_Van < van_best:
            np.save("/data3/zjy/S4NN/clear_background/log1/weights_van_best", W, allow_pickle=True)
            van_best = test_Van
        if test_MSE < mse_best:
            np.save("/data3/zjy/S4NN/clear_background/log1/weights_mse_best", W, allow_pickle=True)
            mse_best = test_MSE
        if test_MAE < mae_best:
            np.save("/data3/zjy/S4NN/clear_background/log1/weights_mae_best", W, allow_pickle=True)
            mae_best = test_MAE
